import { Menu, MenuButton, MenuList, MenuItem, Button } from '@chakra-ui/react'
import { TaskPriority } from '../../../../types/types'
import { PRIORITY_COLORS } from '../../../../config'

const PRIORITY_LABELS: Record<TaskPriority, string> = {
	[TaskPriority.HIGH]: 'High',
	[TaskPriority.MEDIUM]: 'Medium',
	[TaskPriority.LOW]: 'Low',
	[TaskPriority.NONE]: 'None',
}

type PriorityFormProps = {
	onSelectPriority: (priority: TaskPriority) => void
	selectedPriority: TaskPriority
}

const PriorityForm: React.FC<PriorityFormProps> = ({ onSelectPriority, selectedPriority }) => {
	return (
		<Menu matchWidth={true}>
			<MenuButton bg={!selectedPriority ? 'transparent' : PRIORITY_COLORS[selectedPriority]} as={Button}>
				{selectedPriority ? PRIORITY_LABELS[selectedPriority] : '-'}
			</MenuButton>

			<MenuList p={2}>
				{Object.values(TaskPriority).map(priority => (
					<MenuItem
						mb={2}
						key={priority}
						as={Button}
						bg={PRIORITY_COLORS[priority]}
						onClick={() => onSelectPriority(priority)}>
						{PRIORITY_LABELS[priority]}
					</MenuItem>
				))}
			</MenuList>
		</Menu>
	)
}

export default PriorityForm
